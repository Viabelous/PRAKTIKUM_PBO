...
/* [ Versi Program 02 ]
   ...
   -. Serbuan posttest di hari yang sama membuat
      tidak sempatnya error handling (unique atribute)
      pada registrasi akun dilaksanakan

   -. Keranjang belum dibuat spesifik untuk pengguna
...
public class main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean admin = false, unEmpt = false, stats1 = true, stats2 = true;
        String Username, Password, activeUser = "";
        ...
        // Admin spesial corner
        dataAkun.add(new akun(0, "Yafi", "Viabel", "Viabel", "StressPosttest@gmail.com"));
        ...
        while(stats1){
            ...
            while(stats2){
                ...
                switch(opsi){
                    case 1:
                        ...
                        if(index != -1){
                            if(dataAkun.get(index).getPass().equals(Password)){
                                ...
                                if(activeUser.equals(dataAkun.get(0).getUsn())){
                                    admin = true;
                                }
                                stats2 = false;
                                cls();
                    ...
                    case 2:
                        if(!dataAkun.isEmpty()){
                            index = dataAkun.get(dataAkun.size() - 1).getID() + 1;
                        } else{
                            index = 0;
                        }
                        dataAkun.add(new akun(index, null, null, null, null));
                        index = SequentialSearch(dataAkun, index);
                        try{
                            TambahAkun(dataAkun, index);
                        } catch (Exception e){
            ...
            if(admin == true){
                ...
                System.out.println("| Selamat Datang " + activeUser + "!\n" 
                         + "| Silakan pilih menu yang dituju\n"
                         + "| --[1] Lihat Daftar Hewan\n"
                         + "| --[2] Lihat Daftar Perlengkapan\n"
                         + "| --[3] Keranjangku\n"
    ...
    static void TambahAkun(ArrayList <akun> Akun, int Index) throws IOException{
        InputStreamReader isr = new InputStreamReader(System.in);
        BufferedReader br = new BufferedReader(isr);
        ...
        Nama = br.readLine();
        Akun.get(Index).setNama(Nama);
        
        System.out.print(" Usn      : ");
        Usn = br.readLine();
        Akun.get(Index).setUsn(Usn);        
        
        System.out.print(" Pass     : ");
        Pass = br.readLine();
        Akun.get(Index).setPass(Pass);
        
        System.out.print(" Email    : ");
        Email = br.readLine();
        Akun.get(Index).setEmail(Email);
    }
